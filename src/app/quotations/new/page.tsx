...
export default function CreateQuotationPage() {
  ...
  const [lineItems, setLineItems] = useState<LineItemState[]>([
    { productId: "", quantity: 1, price: 0, description: "" },
  ]);
  ...
  const {
    ...
    loading: dataLoading,
    error: dataError,
  } = useQuery<GetQuotationFormData>(GET_FORM_DATA);
  ...
  const [createQuotation, { loading: mutationLoading, error: mutationError }] =
    useMutation(CREATE_QUOTATION, {
      ...
      refetchQueries: ["GetDashboardData", { query: GET_QUOTATIONS }],
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const finalLineItems = lineItems
      .map(({ productId, quantity, price, description }) => ({
        productId,
            ...
            >
              <thead style={{ borderBottom: "1px solid #e5e7eb" }}>
                <tr>
                  ...
                  <th style={{ ...tableHeaderStyle, width: "20%" }}>
                    Price (AED)
                  </th>
                  <th
                    style={{
    ...
    >
      ...
      >
        ...
        >
          {isSuccess ? (
            <svg
              style={{ width: "24px", height: "24px", color: iconColor }}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
...
const FormSection = ({
  ...
  <div style={formSectionStyle}>
    {" "}
    <h2 style={sectionHeaderStyle}>{title}</h2> {children}{" "}
  </div>
);
const InputField = ({
  label,
