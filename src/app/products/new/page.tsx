...
export default function NewProductPage() {
  ...
  const [modalOnClose, setModalOnClose] = useState(() => () => {});
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        // Explicitly build the input object to handle optional fields correctly
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <FormSection title="Product Information">
                    <div style={gridStyle}>
                        ...
                        <InputField label="Price (AED)" name="price" type="number" step="0.01" value={formData.price} onChange={handleChange} required placeholder="e.g., 1500.00" />
                        
                        {/* --- FIX: Input field now uses 'productId' and has the generate button --- */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            <InputField label="Product ID (Optional)" name="productId" value={formData.productId} onChange={handleChange} placeholder="e.g., APP-001" />
                            <button 
                                type="button" 
...
const gridStyle: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', alignItems: 'start' };
