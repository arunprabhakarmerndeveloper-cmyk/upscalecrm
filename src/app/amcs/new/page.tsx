    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
        <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '0.75rem', width: '90%', maxWidth: '400px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                ...
                {onClose && <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer' }}>&times;</button>}
            </div>
            {children}
        </div>
    </div>
...
const SuccessModal = ({ message, onClose }: { message: string, onClose: () => void }) => (
    <Modal title="Success" onClose={onClose}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
            ...
            <button onClick={onClose} style={{ ...buttonStyle, backgroundColor: '#10b981', width: '100%' }}>OK</button>
        </div>
    </Modal>
);
...
const ErrorModal = ({ message, onClose }: { message: string, onClose: () => void }) => (
    <Modal title="Error" onClose={onClose}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
            ...
            <button onClick={onClose} style={{ ...buttonStyle, backgroundColor: '#ef4444', width: '100%' }}>Close</button>
        </div>
    </Modal>
);
...
export default function CreateAmcPage() {
  ...
  const [modal, setModal] = useState<{ type: 'success' | 'error' | null; message: string; data?: any }>({ type: null, message: '', data: null });
  ...
  useEffect(() => {
    if (invoiceData?.invoice && formData) {
        ...
        const productsFromInvoice = invoice.lineItems
            ?.map(item => ({
        ...
        if (productsFromInvoice.length > 0) {
            ...
            setProductInstances([{ productId: '', serialNumber: '', purchaseDate: new Date().toISOString().split('T')[0] }]);
        }
    }
  ...
  const handleModalClose = () => {
      ...
      setModal({ type: null, message: '', data: null });
  };
  ...
  const handleServiceDateChange = (index: number, value: string) => { const updatedVisits = [...serviceVisits]; updatedVisits[index].scheduledDate = value; setServiceVisits(updatedVisits); };
  const handleProductChange = (index: number, field: keyof ProductInstance, value: string) => { const updatedInstances = [...productInstances]; (updatedInstances[index] as any)[field] = value; setProductInstances(updatedInstances); };
  ...
  const productsAreFromInvoice = fromInvoiceId && productInstances.some(p => p.productId !== '');
  ...
  return (
    <div style={{ maxWidth: '800px', margin: 'auto', padding: '2rem 1rem' }}>
        ...
        {modal.type === 'success' && <SuccessModal message={modal.message} onClose={handleModalClose} />}
        {modal.type === 'error' && <ErrorModal message={modal.message} onClose={handleModalClose} />}
...
/>
        ...
        <FormSection title="Contract Details">
          <div style={gridStyle}>
            ...
            <InputField label="Services per Year" name="frequencyPerYear" type="number" min="1" max="12" value={frequencyPerYear} onChange={(e) => setFrequencyPerYear(e.target.value)} required />
          </div>
        </FormSection>
...
interface SelectFieldProps extends React.SelectHTMLAttributes<HTMLSelectElement> { label: string; options?: SelectOption[]; }
